...
%\lipsum[2]
  ...
  Which means
  that these variables are dependant, but have a zero covariance. 
  }
%% \problemAnswer{
%% \begin{center}
%% \includegraphics[width=0.75\columnwidth]{Problem2/example_figure} % Example image
...
\begin{homeworkProblem}
  ...
  \begin{figure}[H]
  \begin{center}
    \includegraphics[width=0.7\columnwidth]{Problem4/P4F1.pdf}
    \vspace{-5mm}
    \caption{Square rejection sampling for first $10^3$ points (top).
             Triangular rejection sampling for first $10^3/2$ points
             (bottom).}
    \label{fig:P4F1}
  \end{center}
  \end{figure}
...
\begin{homeworkProblem}
  ...
  \problemAnswer{
    ...
    \begin{equation*}
      \mu=E[x]=\int_{-\infty}^\infty xf(x)dx
    \end{equation*}
    Solving for the log-normal distribution mean:
    \begin{equation*}
      \mu=E[x]=\int_{-\infty}^\infty
               \frac{1}{\sigma\sqrt{2\pi}}e^{-\frac{((ln(x)-\mu)^2}{2\sigma^2}}dx
    \end{equation*}
