...
\begin{homeworkProblem}
  ...
  Where:
  \begin{equation*}
    \phi(x')=2\pi\sum_{n=0}^{\infty}c_nL_n^{(\alpha)}(\beta x')
  \end{equation*}
  and
  \begin{equation*}
    c_n=\frac{n!}{\Gamma(n+\alpha+1)}\int_0^{\infty}
    \phi\left(\frac{x\cdot z}{\beta}\right)
  ...
  \begin{equation*}
    f(z)=\phi\left(\frac{x\cdot z}{\beta}\right)L_n^{(\alpha)}(z)
  \end{equation*}
  ...
  \begin{align*}
    ...
    \sum_{i=1}^{n'}\frac{\Gamma(n'+\alpha)z_i}
        {n'!(n'+\alpha)(L_{n'-1}^{\alpha}(z_i))^2}
        \phi\left(\frac{x\cdot z_i}{\beta}\right)L_n^{(\alpha)}(z_i)\\
  \end{align*}
  Where $n'$ will increase until the summation is not changing,
  and $n$ is the constant solving for. This way, the $L_n^\alpha(z_i)$
  term on the right side will only be zero for when $n'=n$.
  ...
  \begin{align*}
    ...
    &\approx\sum_{i=1}^{n'}\frac{\Gamma(n'+\alpha)z_i}
        {n'!(n'+\alpha)(L_{n'-1}^{\alpha}(z_i))^2}
        \phi\left(\frac{x\cdot z_i}{\beta}\right)\\
  \end{align*}

  To check if this is correct, we can change the $\phi$
  term for $cos(z/\beta)$ with Z $\sim$ G(1,2) and check to
  see if $c_0$ converges to 0.48. This is what Dr. McClarren
