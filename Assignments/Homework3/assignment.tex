...
\begin{homeworkProblem}
  ...
  \problemAnswer{
    ...
    \begin{align*}
      ...
      \right)\\
      &=QXYZ+YZL(e^{X/L}-1)+XZL(e^{Y/L}-1)+XYL(e^{Z/L}-1)
    \end{align*}
    ...
    \begin{equation*}
      ...
      =\boxed{QYZ+YZe^{X/L}+ZL(e^{Y/L}-1)+YL(e^{Z/L}-1)}
    \end{equation*}
    \begin{equation*}
      \frac{\delta QoI}{\delta Y}
      =\boxed{QXZ+ZL(e^{X/L}-1)+XZe^{Y/L}+XL(e^{Z/L}-1)}
    \end{equation*}
    \begin{equation*}
      \frac{\delta QoI}{\delta Z}
      =\boxed{QXY+YL(e^{X/L}-1)+XL(e^{Y/L}-1)+XYe^{Z/L}}
    \end{equation*}
    \begin{equation*}
      \frac{\delta QoI}{\delta L}
      =\boxed{YZe^{X/L}(1-e^{-X/L}-\frac{X}{L})
...
\begin{homeworkProblem}
  ...
  It should be noted that I shuffled the stratified samples and the
  halton samples as well, because otherwise there would be some
  correlation between the numbers. The first order listing in the
  table is not from the code, but the calculation above, and did
  not use 50 points.
...
\begin{homeworkProblem}
  ...
  \problemAnswer{
    \begin{align*}
      ...
      &=2\pi \int_\infty^1-\frac{e^{-\sigma xz}}{z^2}\\
      &=2\pi \int_1^\infty\frac{e^{-\sigma xz}}{z^2}\\
      &=2\pi E_2(\sigma x)
    \end{align*}
    I think this might help later on, if we define $\phi$ like:
    \begin{equation*}
      \phi^+(x')=2\pi E_2(x')
    \end{equation*}
    where $x'=\sigma x$.
  ...
  Where:
  \begin{equation*}
    \phi(x')=\sum_{n=0}^{\infty}c_nL_n^{(\alpha)}(\beta x')
  \end{equation*}
  and
  \begin{equation*}
    c_n=\frac{n!}{\Gamma(n+\alpha+1)}\int_0^{\infty}
    \phi\left(\frac{z}{\beta}\right)
    z^\alpha e^{-z}L_n^{(\alpha)}(z)dz
  \end{equation*}

  In order to estimate the coefficients in the expansion we have to
  evaluate 
  a wonderful integral. In order to estimate the integral, will
  ...
  \begin{equation*}
    f(z)=\phi\left(\frac{z}{\beta}\right)L_n^{(\alpha)}(z)
  \end{equation*}

  I hope this is correct, if not then the only person I can
  blame is myself. The notes aren't very clear on what $f(z)$ is.
  If I am correct, then the $c_n$ constants are:
  ...
  I am not sure how to get this other than sampling.
