...
\begin{homeworkProblem}
  ...
  \problemAnswer{
    ...
    problem 5, vary the alpha and beta parameters, and see
    which gives me the smallest total error. I know its dumb,
    and its not actually correct, because I'm not finding
    a posterior as Bayes would want (I am just changing the
    gamma variable to fit the data and throwing out the
    prior), but I'm just a guy. \\
  }

  After looking for a parameter that would minimize the
  error between measured values and calculated values,
  I got $\alpha=$ and $\beta=$.
